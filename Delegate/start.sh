...
for (( ;; )); do
        ...
        echo -e "${PWDDD}\ny\n" | ${PROJECT} tx distribution withdraw-rewards --from ${WALLETNAME} --chain-id ${CHAIN_ID}  --gas ${GAS} --gas-prices ${GAS_PRICE} --gas-adjustment ${GAS_AD}
...
echo -e "BALANCE: ${GREEN_COLOR}${BAL}${WITHOU_COLOR} $DENOM\n"
        ...
        echo -e "${PWDDD}\n${PWDDD}\n" | ${PROJECT} tx distribution withdraw-all-rewards --from ${WALLETNAME} --chain-id ${CHAIN_ID}  --gas ${GAS} --gas-prices ${GAS_PRICE} --gas-adjustment ${GAS_AD}
for (( timer=12; timer>0; timer-- ))
        do
...
echo -e "BALANCE: ${GREEN_COLOR}${BAL}${WITHOU_COLOR} ${DENOM}\n"
        echo -e "Stake ALL\n"
if (( BAL > 500 )); then
            echo -e "${PWDDD}\n${PWDDD}\n" | ${PROJECT} tx staking delegate ${VAL_ADDR} ${BAL}${DENOM} --from ${WALLETNAME} --chain-id ${CHAIN_ID} --gas-adjustment ${GAS_AD} --gas=${GAS} --gas-prices=${GAS_PRICE} 
